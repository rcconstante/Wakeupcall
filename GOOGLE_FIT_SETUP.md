...
```
1. User clicks "Grant Permission" in app
   ↓
2. Android Google Play Services shows Google Sign-In
   ↓
3. User selects Google account
   ↓
4. Google Fit permission dialog appears
   ↓
5. User grants permissions
   ↓
6. App receives GoogleSignInAccount
   ↓
7. App uses Fitness API to fetch data:
   - DataType.TYPE_STEP_COUNT_DELTA (daily steps)
   - DataType.TYPE_SLEEP_SEGMENT (sleep duration)
   - DataType.TYPE_ACTIVITY_SEGMENT (activity types)
   ↓
8. Data displayed in Profile screen charts
   ↓
9. Data used in survey submission for ML predictions
```
...
1. **Local Storage Only**
   - Google Fit data is fetched on-demand
   - Not permanently stored in your database
   - Only aggregated metrics sent to backend for ML

2. **User Control**
   - Users can disconnect anytime via "Data Sources" screen
   - Revoking permission removes app's access immediately

3. **Minimal Permissions**
   - App only requests READ access
   - Cannot write or modify user's Google Fit data

4. **Transparency**
   - Data Consent screen clearly explains what's accessed
   - Google's permission dialog shows exactly what's shared
...
Before releasing your app to Google Play:

### 1. OAuth Verification (Required for production)
- [ ] Submit app for OAuth verification in Google Cloud Console
- [ ] Provide privacy policy URL
- [ ] Provide terms of service URL
- [ ] Explain why you need Google Fit access
- [ ] Verification process takes 3-7 business days

### 2. Release Keystore
- [ ] Generate release keystore:
  ```powershell
  keytool -genkey -v -keystore wakeupcall-release.keystore -alias wakeupcall -keyalg RSA -keysize 2048 -validity 10000
  ```
- [ ] Get SHA-1 from release keystore
- [ ] Add release SHA-1 to Google Cloud Console OAuth client
...
android {
    signingConfigs {
        release {
            storeFile file("wakeupcall-release.keystore")
...
- What Google Fit data you collect
...
- [ ] Declare Google Fit permissions usage
- [ ] Submit for review
...
├── googlefit/
│   └── GoogleFitManager.kt          # Core Google Fit API wrapper
├── viewmodel/
│   └── GoogleFitViewModel.kt        # State management for Fit data
└── frontend/
    ├── DataSources.kt               # Connect/Disconnect UI
    ├── Profile.kt                   # Sleep & Steps charts
    └── DataConsentScreen.kt         # Permission request
```

### Important URLs:
- Google Cloud Console: https://console.cloud.google.com/
- Fitness API Docs: https://developers.google.com/fit/android
...
adb shell pm clear com.example.wakeupcallapp.sleepapp

# Check logcat for Google Fit errors
adb logcat | Select-String "GoogleFit"
```
...
**Q: Do I need to pay for Google Fit API?**
A: No, Google Fit API is free for reasonable usage limits.

**Q: Can I test without a physical device?**
A: Emulators work but may not have realistic data. Physical device recommended.

**Q: How often does data sync?**
A: Google Fit syncs automatically. Force sync by opening Google Fit app.

**Q: What if user doesn't have Google Fit?**
A: App works without it - Google Play Services handles Fit data automatically.

**Q: Can I access historical data?**
A: Yes, you can fetch data from past 7, 30, or more days.

---

## ✅ Setup Complete!

Your app now has full Google Fit integration:
- ✅ OAuth authentication configured
- ✅ Permission flow implemented  
- ✅ Real-time step data
- ✅ Sleep tracking with charts
- ✅ Activity data for ML predictions
- ✅ Connect/Disconnect functionality

**Next Steps:**
1. Test with your Google account on a physical device
2. Walk around to generate step data
3. View data in Profile screen
4. Test disconnect/reconnect in Data Sources
5. Submit survey to see Google Fit data improve ML predictions
...
- Stack Overflow: Tag `google-fit-sdk` and `android`
- Check app logs: `adb logcat | Select-String "GoogleFitManager"`
